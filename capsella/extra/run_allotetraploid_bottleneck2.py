...
"""
<< run_allotetraploid_bottleneck2.py >>
...
def allotetraploid_bottleneck2(params, ns, pts):
    """
    params = (nu01,nu02,nuBot,T1,T2,misid)
    ns = (n1,n2)
    ...
    """
    nu01, nu02, nuBot, T1, T2,misid = params
...
if __name__ == "__main__":
    ...
    f_out = open("capsella_allotetraploid_bottleneck2.csv", 'w')
    print(
        "loglik", "nu01", "nu02", "nuBot", "T1", "T2", "misid", "theta",
        sep=",", file=f_out
    )
    ...
    pts_l = [50,60,70]
    func = allotetraploid_bottleneck2
    func_ex = dadi.Numerics.make_extrap_log_func(func)

    # Set bounds
    # nu01,nu02,nuBot,T1,T2,misid
    upper_bound = [100, 100, 100, 100, 100, 1.0]
    lower_bound = [1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 0.0]

    # Guess at initial parameter values
    p_init = [10.0, 20.0, 0.2, 2.0, 0.5, 0.05]
    ...
    while opt_successes < optimization_runs and opt_failures < max_failures:
        ...
        print(
            LLopt, popt[0], popt[1], popt[2], popt[3], popt[4], popt[5],
